...
image_name = [
%    'kodim12.png'
%    'syntetic_image_new_noisy_result_s5_run_through_pipe8.tif'
   'syntetic_image_noisy_result_s5_run_through_pipeline8bit.tif'
...
%%%%
if (exist('sigma') ~= 1),
   sigma                = 13; %%13 idi 5 idi 50idi 30 idi  1 idi default standard deviation of the AWGN
end
...
%     imwrite(min(max(zRGB,0),1), 'syntetic_image_noisy_result_s5.tif');
    figure, imshow(min(max(yRGB_est,0),1)); title(sprintf('Denoised %s, PSNR: %.3f dB', ...
        image_name(1:end-4), PSNR));
    imwrite(min(max(yRGB_est,0),1), 'syntetic_image_noisy_result_s5_run_through_pipeline8bit_denoiseds13.tif');
end
